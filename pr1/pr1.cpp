    ...
    for (int current_N : N_values) {
        for (int current_L : L_values) {
            ...
            // ПОСЛЕДОВАТЕЛЬНАЯ ВЕРСИЯ
            {
                ...
                results.push_back({ "Sequential", current_N, current_L, duration_a, duration_b, duration_c, duration_d,
            ...
            // ПАРАЛЛЕЛЬНАЯ ВЕРСИЯ
            {
                ...
                // Инициализация массива a (параллельно)
                array_a[0] = x;
#pragma omp parallel for
                ...
                // Инициализация массива b (последовательно)
                array_b[0] = 0.0;
                for (int i = 1; i < current_N; ++i) {
                    double temp_value = array_b[i - 1];
                    for (int rep = 0; rep < current_L; ++rep) {
                ...
                // Инициализация массива c (параллельно)
#pragma omp parallel for
                for (int i = 0; i < current_N; ++i) {
                    int reverse_index = current_N - 1 - i;
                    double accumulator = 0.0;
                ...
                // Инициализация массива d (последовательно)
                array_d[0] = x;
                for (int i = 1; i < current_N; ++i) {
                    double current_val = array_d[i - 1];
                    for (int rep = 0; rep < current_L; ++rep) {
                ...
                // Переопределение массива b (параллельно)
                array_b[0] = x;
#pragma omp parallel for
                for (int i = 1; i < current_N; ++i) {
                    double current_val = array_b[i];
                ...
                // Переопределение массива d (параллельно)
#pragma omp parallel for
                for (int i = 0; i < current_N; ++i) {
                    int reverse_index = current_N - 1 - i;
                    double accumulator = 0.0;
                ...
                results.push_back({ "Parallel", current_N, current_L, duration_a, duration_b, duration_c, duration_d,
                                  duration_b_redef, duration_d_redef, total_duration });
            }
        }
    }
    ...
    if (output_file.is_open()) {
        ...
        output_file << "TYPE\tN\tL\ta_time\tb_time\tc_time\td_time\tb2_time\td2_time\ttotal_time\n";
