...
vector<string> readSchedules(const string& prompt) {
    cout << prompt;
    string input_line;
    getline(cin, input_line);
    if (input_line.empty()) {
...
int main() {
    ...
    auto schedule_types = readSchedules("Типы планирования (static dynamic guided): ");
    if (schedule_types.empty()) {
        schedule_types = { "static" };
    }

    cout << fixed << setprecision(2);
    cout << "TYPE\tN\tL\ta_time\tb_time\tc_time\td_time\tb2_time\td2_time\ttotal_time\n";
    ...
    for (int current_N : N_values) {
        for (int current_L : L_values) {
            ...
            results.push_back({ "Sequential", current_N, current_L, 1, "-",
                duration_a, duration_b, duration_c, duration_d,
                duration_b_redef, duration_d_redef, total_duration });
        }
    }

    // ПАРАЛЛЕЛЬНАЯ ВЕРСИЯ С РАЗНЫМИ НАСТРОЙКАМИ
...
#pragma omp parallel for schedule(guided)
                    ...
                    // Инициализация массива b (последовательная)
                    array_b[0] = 0.0;
                    for (int i = 1; i < current_N; ++i) {
                        double temp_value = array_b[i - 1];
                        for (int rep = 0; rep < current_L; ++rep) {
...
#pragma omp parallel for schedule(guided)
                    ...
                    // Инициализация массива d (последовательная)
                    array_d[0] = x;
                    for (int i = 1; i < current_N; ++i) {
                        double current_val = array_d[i - 1];
                        for (int rep = 0; rep < current_L; ++rep) {
...
#pragma omp parallel for schedule(guided)
                    ...
                    string mode_name = "Parallel_" + sched_type + "_k" + to_string(k);
                    cout << mode_name << "\t" << current_N << "\t"
                         << current_L << "\t"
                         << duration_a << "\t"
                         << duration_b << "\t"
                         << duration_c << "\t"
                    ...
                    results.push_back({ "Parallel_" + sched_type, current_N, current_L, k, sched_type,
                        duration_a, duration_b, duration_c, duration_d,
                        duration_b_redef, duration_d_redef, total_duration });
                }
            }
    ...
    // ПАРАЛЛЕЛЬНАЯ ВЕРСИЯ ПО УМОЛЧАНИЮ (как в pvs1)
...
#pragma omp parallel for
            ...
            results.push_back({ "Parallel", current_N, current_L, 2, "static",
                duration_a, duration_b, duration_c, duration_d,
                duration_b_redef, duration_d_redef, total_duration });
        }
    }
    ...
    if (output_file.is_open()) {
        ...
        output_file << "TYPE\tN\tL\tThreads\tSchedule\ta_time\tb_time\tc_time\td_time\tb2_time\td2_time\ttotal_time\n";
